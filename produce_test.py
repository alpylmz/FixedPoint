...
def assignment_test(n, m):
    ...
    for _ in range(int(NUM_TESTS)):
        ...
        with open(output_file, 'a') as f:
            f.write(f"# {num} {float((FixedPoint(num, True, m, n, overflow=OVERFLOW_MODE, rounding='down', overflow_alert='ignore')))}\n")
...
def addition_test(n, m):
    ...
    for _ in range(int(NUM_TESTS)):
        ...
        # convert to fixed point
        fixed_num1 = FixedPoint(num1, True, m, n, overflow=OVERFLOW_MODE, rounding='down', overflow_alert='ignore')
        fixed_num2 = FixedPoint(num2, True, m, n, overflow=OVERFLOW_MODE, rounding='down', overflow_alert='ignore')
        summ = float(FixedPoint(float(fixed_num1 + fixed_num2), True, m, n, overflow=OVERFLOW_MODE, rounding='down', overflow_alert='ignore'))
        # write to file
        with open(output_file, 'a') as f:
...
def subtraction_test(n, m):
    ...
    for _ in range(int(NUM_TESTS)):
        ...
        fixed_num1 = FixedPoint(num1, True, m, n, overflow=OVERFLOW_MODE, rounding='down', overflow_alert='ignore')
        fixed_num2 = FixedPoint(num2, True, m, n, overflow=OVERFLOW_MODE, rounding='down', overflow_alert='ignore')
        sub = float(FixedPoint(float(fixed_num1 - fixed_num2), True, m, n, overflow=OVERFLOW_MODE, rounding='down', overflow_alert='ignore'))
        # write to file
        with open(output_file, 'a') as f:
...
def multiplication_test(n, m):
    ...
    for _ in range(int(NUM_TESTS)):
        ...
        fixed_num1 = FixedPoint(num1, True, m, n, overflow=OVERFLOW_MODE, rounding='down', overflow_alert='ignore')
        fixed_num2 = FixedPoint(num2, True, m, n, overflow=OVERFLOW_MODE, rounding='down', overflow_alert='ignore')
        mult = float(FixedPoint(float(fixed_num1 * fixed_num2), True, m, n, overflow=OVERFLOW_MODE, rounding='down', overflow_alert='ignore'))
        # write to file
        with open(output_file, 'a') as f:
...
def unary_minus_test(n, m):
    ...
    for _ in range(int(NUM_TESTS)):
        ...
        num = (random() * (MAX_BOUND - MIN_BOUND)) + MIN_BOUND
        # convert to fixed point
        fixed_num =float( FixedPoint(num, True, m, n, overflow=OVERFLOW_MODE, rounding='down', overflow_alert='ignore'))
        # write to file
        with open(output_file, 'a') as f:
...
for N in tqdm(range(20, 21)):
    for M in tqdm(range(20, 21)):
        #print("N: ", N, "M: ", M)
        test_num(N, M)
...
# example
custom_fixed = FixedPoint(-130.622, True, m=8, n=8, overflow=OVERFLOW_MODE, rounding='down', overflow_alert='ignore')
print(float(custom_fixed))
num2 = FixedPoint(-211.598, True, m=8, n=8, overflow=OVERFLOW_MODE, rounding='down', overflow_alert='ignore')
print(float(num2))
print(float(FixedPoint(float(custom_fixed + num2), True, m=8, n=8, overflow=OVERFLOW_MODE, rounding='down', overflow_alert='ignore')))
"""
